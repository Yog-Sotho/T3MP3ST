...
## 2026-08-01 - Prototype Lookup Bypass in Dynamic Connector Registry
**Vulnerability:** The `getConnector` function resolved connector instances from the `CONNECTORS` registry using direct bracket access (`CONNECTORS[platform]`). Since the object was instantiated as a standard JavaScript object literal, an attacker supplying prototype-pollution payloads or prototype keys (such as `__proto__`, `constructor`, or `toString`) could bypass intended key validations, potentially leading to prototype property lookup or unexpected execution side effects on the registry.
**Learning:** Standard object literals carry inherited properties on their prototypes. Relying on simple bracket lookups without checking property ownership allows access to base object functions/properties.
**Prevention:** Always instantiate dynamic registry objects with `Object.create(null)` to completely strip the prototype, and validate incoming lookup keys using `Object.prototype.hasOwnProperty.call`.
