...
## 2026-08-12 - SSRF Bypass via Port-Appended Unbracketed IPv4-Mapped/Compatible IPv6 Addresses
**Vulnerability:** The `isRestrictedInternalIP` function stripped port suffixes from standard IPv4 addresses and single-colon hosts, but failed to strip port suffixes from unbracketed IPv4-mapped and IPv4-compatible IPv6 addresses (e.g., `::ffff:127.0.0.1:8080` or `0:0:0:0:0:0:127.0.0.1:8080`). As a result, the trailing port colon prevented matching standard IPv4 patterns or parsing alternative IPv4 formats, leading to an SSRF bypass.
**Learning:** Port stripping logic that only checks for a single colon (`firstColon === lastColon`) skips unbracketed IPv6 addresses because IPv6 addresses contain multiple colons.
**Prevention:** Explicitly strip port suffixes from unbracketed IPv4-mapped/compatible IPv6 host strings (`/^(?:(?:0*:){1,6}|::)(?:ffff:(?:0:)?)?([^:]+)(?::\d{1,5})$/i`) before parsing or validating the embedded IPv4 address.

## 2026-08-13 - SSRF Bypass via IPv6 Unique Local, Link-Local, and Multicast Prefix Truncation
**Vulnerability:** The `isRestrictedInternalIP` function checked for IPv6 unique local (`fc00::`) and link-local (`fe80::`) addresses, but used overly restrictive regexes (`/^(fc00|fd00):/` and `/^fe80:/`). As a result, valid ULA addresses in the full RFC 4193 `fc00::/7` block (such as `fd12::`), link-local addresses in `fe80::/10` (such as `fe90::`), and multicast addresses in `ff00::/8` (such as `ff02::`) bypassed internal IP filters and allowed SSRF attacks.
**Learning:** IPv6 range checks using exact 16-bit word prefixes (like `fe80:`) miss addresses within the wider CIDR block (like `fe80::/10` or `fc00::/7`) when non-zero subnet bits are present in the second or third nibble.
**Prevention:** Use range-aware regex patterns matching full CIDR blocks (e.g., `/^f[cd][0-9a-f]{2}:/i` for `fc00::/7`, `/^fe[8-f][0-9a-f]:/i` for `fe80::/10`, and `/^ff[0-9a-f]{2}:/i` for `ff00::/8`).

## 2026-08-14 - SSRF Bypass via Hex-Word Formatted IPv4-Mapped/Compatible IPv6 Addresses
**Vulnerability:** The `isRestrictedInternalIP` function evaluated IPv4-mapped (`::ffff:`) and IPv4-compatible (`::`) IPv6 addresses, but failed to parse addresses where the embedded IPv4 address was formatted in 16-bit hex words (e.g. `::ffff:7f00:1` for `127.0.0.1` or `::ffff:a9fe:a9fe` for `169.254.169.254`). Because OS network resolvers and Node socket libraries resolve hex-word IPv4-mapped/compatible IPv6 addresses directly to internal IPv4 targets, this allowed SSRF filters to be bypassed.
**Learning:** Embedded IPv4 addresses within IPv6 mapped or compatible addresses can be represented using two 16-bit hex words instead of standard dot-decimal notation. Checking for embedded dot-decimal strings alone fails to intercept valid hex-word IPv6 representations that route to restricted loopback, private, or metadata IPs.
**Prevention:** Convert hex-word IPv4-mapped/compatible IPv6 tail segments (`/^([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i`) to 32-bit integers and format them into standard dot-decimal IPv4 representation before running private and loopback IP range checks.

## 2026-08-15 - SSRF Bypass via Port-Appended Hex-Word Formatted IPv4-Mapped/Compatible IPv6 Addresses
**Vulnerability:** The `isRestrictedInternalIP` function parsed hex-word formatted IPv4-mapped (`::ffff:7f00:1`) and IPv4-compatible (`::7f00:1`) IPv6 addresses, but failed to strip port suffixes when appended to hex-word addresses (e.g. `::ffff:7f00:1:8080`). The trailing port colon caused `potentialIp` to contain three colon-separated hex segments (`7f00:1:8080`), failing the 2-group hex-word regex `/^([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i` and bypassing SSRF checks.
**Learning:** Embedded hex-word IPv4 representations in IPv6 addresses can carry trailing port suffixes (`:port`), creating an extra colon-delimited segment that invalidates standard 2-segment hex-word matching logic unless stripped beforehand.
**Prevention:** Explicitly strip port suffixes from hex-word embedded IPv4 tail strings (`potentialIp = potentialIp.replace(/^([0-9a-f]{1,4}:[0-9a-f]{1,4})(?::\d{1,5})$/i, '$1')`) after IPv6 prefix removal before running 2-group hex-word parsing.
