...
## 2026-08-14 - Unfiltered Option Flags in Whitelisted Binaries (Naabu, ExifTool, Checkov)
**Vulnerability:** Whitelisted binaries `naabu`, `exiftool`, and `checkov` lacked `dangerousFlags` regex rules in `parseCommand`, allowing attackers to pass options like `exiftool -config <file>` (arbitrary Perl code execution), `checkov --external-checks-dir <dir>` (arbitrary Python check execution), or output file options (`-o`, `-f`, `-dL`) to read/write arbitrary local files when calling `/api/tools/execute`.
**Learning:** Adding new tools to `SAFE_COMMANDS` without auditing their command-line arguments for configuration files, external script directories, or output paths introduces command execution and arbitrary file access vulnerabilities.
**Prevention:** Every executable in `SAFE_COMMANDS` that accepts custom configuration files, script/check directories, file reads, or file output options must have an explicit `dangerousFlags` regex filtering out those options.
