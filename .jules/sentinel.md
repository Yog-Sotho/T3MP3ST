...
## 2026-08-03 - IPv6 Zone Index SSRF Protection Bypass and MCP Server Hardening
**Vulnerability:** The `isRestrictedInternalIP` function failed to handle IPv6 zone indices (e.g. `[::1%eth0]`), which could allow attackers to bypass loopback IP checks in underlying network libraries. Additionally, the standalone MCP server (`src/mcp-server.ts`) did not check targets against `isRestrictedInternalIP` prior to executing reconnaissance tools, making it susceptible to server-side request forgery (SSRF).
**Learning:** Checking IPv6 loopback addresses via exact string matching is vulnerable to zone-indexed variations that resolvers support natively. Furthermore, security controls must be consistently applied across all server components, including secondary/standalone entrypoints like an MCP server.
**Prevention:** Always strip IPv6 zone indices (splitting on `%`) before checking IP ranges. Consistently validate all client-supplied targets in standalone tools (such as MCP endpoints) against the unified `isRestrictedInternalIP` policy.
