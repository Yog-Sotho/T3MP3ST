...
## 2026-08-11 - SSRF Bypass via Port-Appended IPv4-Mapped/Compatible IPv6 Addresses
**Vulnerability:** The `isRestrictedInternalIP` function stripped port numbers from single-colon IPv4 or hostname strings, but skipped port stripping when multiple colons were present unless wrapped in brackets `[...]`. Unbracketed IPv4-mapped or IPv4-compatible IPv6 addresses with ports (e.g. `::ffff:127.0.0.1:8080`, `::127.0.0.1:8080`) retained their trailing `:port` string, causing regexes and IP parsers to fail matching loopback/private ranges and resulting in an SSRF bypass.
**Learning:** Checking for port numbers based solely on `firstColon === lastColon` assumes all multi-colon strings are standard IPv6 addresses without ports. IPv4-mapped/compatible IPv6 host representations contain both colons and dot-decimal IPv4 address components, requiring port stripping when the host part matches mapped/compatible prefixes or contains dots.
**Prevention:** Ensure port stripping evaluates whether the substring prior to the final colon matches IPv4-mapped/compatible IPv6 patterns or contains dot-decimal IP components before performing address validation.
