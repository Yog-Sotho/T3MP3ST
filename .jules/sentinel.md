...
## 2026-08-14 - SSRF Bypass via Hex-Word IPv4-Mapped/Compatible IPv6 Addresses
**Vulnerability:** The `isRestrictedInternalIP` function stripped IPv6 prefixes from IPv4-mapped and IPv4-compatible IPv6 addresses, but `parseAlternativeIPv4` failed to handle embedded IPv4 addresses formatted as two 16-bit hexadecimal words (e.g., `::ffff:7f00:1` or `::ffff:a00:1`). Node's `net.connect` and OS network resolvers resolve hex-word IPv6 representations to standard IPv4 loopback or private addresses (e.g., `127.0.0.1`), allowing SSRF protection bypass.
**Learning:** IPv4 addresses embedded in IPv6 representations can be formatted as hexadecimal words (`<hex1>:<hex2>`) in addition to standard dot-decimal notation.
**Prevention:** In alternative IPv4 parsing logic, detect two 16-bit hex words (`/^[0-9a-f]{1,4}:[0-9a-f]{1,4}$/i`) and convert both words to standard dot-decimal IPv4 representation before executing private/loopback IP range checks.
