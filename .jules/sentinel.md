...
## 2026-08-11 - Discovery Scanner Flag Injection in Direct Command Execution
**Vulnerability:** The `/api/tools/execute` endpoint allowed command execution for whitelisted discovery binaries (`feroxbuster`, `httpx`, `subfinder`, `katana`, `naabu`), but `parseCommand` lacked `dangerousFlags` filters for them. This allowed attackers to perform flag injection to read/write arbitrary local files or import external configurations via options like `-o`, `--output-dir`, `-c`, `-r`, `-l`, or `-dL`.
**Learning:** Adding binaries to the `SAFE_COMMANDS` whitelist without corresponding `dangerousFlags` regex rules leaves file read/write and config loading vectors open when direct execution arguments are accepted.
**Prevention:** Always define explicit `dangerousFlags` regex patterns in `parseCommand` whenever a new CLI tool accepting file or configuration options is added to `SAFE_COMMANDS`.
