...
## 2026-07-31 - Bug Bounty Platform Prototype Bypass
**Vulnerability:** The bug bounty platform registry used a standard JS object map `CONNECTORS[platform]` and validated keys against a hardcoded array in `validateBountyCredentials`. Because of JavaScript's prototype inheritance, keys like `toString`, `valueOf`, or `__proto__` would resolve to built-in functions or objects on the dictionary, potentially bypassing validation or leading to runtime type errors if called directly as platform names on the API.
**Learning:** Dictionary lookups using standard object literals can resolve built-in inherited methods/properties if user input is not verified against own properties.
**Prevention:** Always instantiate dictionary/registry maps using `Object.create(null)` to eliminate standard prototype lookups, verify keys with `Object.prototype.hasOwnProperty.call`, and validate dynamic lists with own-property arrays via `listConnectors()`.
