...
## 2026-08-15 - Whitelisted Binary Flag Injection via ExifTool, Semgrep, Checkov, and OpenSSL Config Evaluation
**Vulnerability:** Whitelisted binaries (`exiftool`, `semgrep`, `checkov`, `gitleaks`, `trivy`, `openssl`, `binwalk`, `yara`) lacked flag injection guards in `parseCommand`, allowing arbitrary code execution or local file disclosure/writes via flags like `exiftool -config` (Perl code evaluation) or `openssl -config` (engine/module loading) when passed to `/api/tools/execute`.
**Learning:** Whitelisting command-line tools that support configuration file loading (`-config`, `--config-file`) or script evaluation can lead to arbitrary code execution if flags are not explicitly filtered.
**Prevention:** Maintain explicit `dangerousFlags` regex filters blocking `-config`, `-c`, `-eval`, `-o`, and script/module options for all whitelisted tools in `SAFE_COMMANDS`.
