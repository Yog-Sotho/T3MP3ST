...
## 2026-09-12 - [Contextual ARIA Labels and Keyboard Access on Dynamic Roster Cards]
**Learning:** In dynamic component grids (like specialist operative roster cards), generic button labels such as "▶ Deploy" or "✕ Recall" lack entity context for screen-reader users, and container divs with click handlers lack tab focusability. Supplying dynamic `aria-label` and `title` attributes that include the operative name (e.g. "Deploy Recon specialist"), along with `tabindex="0"`, `role="button"`, and `Enter`/`Space` keydown handlers on the card container, provides complete keyboard navigation and clear auditory context.
**Action:** When generating dynamic card grids with interactive controls, always include target entity names in action button ARIA labels and ensure card container divs have standard keyboard focus and keydown activation handlers.
