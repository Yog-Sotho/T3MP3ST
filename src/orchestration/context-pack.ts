...
// ⚡ BOLT OPTIMIZATION: Pre-compile SECURITY_HINTS into case-insensitive RegExp objects
// to completely eliminate the need to lowercase path or content for checking these hints.
const SECURITY_HINTS_RES = SECURITY_HINTS.map(hint => new RegExp(hint, 'i'));
 ...
 *
 * ⚡ BOLT OPTIMIZATION: Refactored to accept pre-compiled RegExp objects instead of string keywords.
 * This completely avoids allocating large lowercase copies of file content for case-insensitive matches,
 * significantly saving memory and garbage collection overhead.
 */
function scoreFile(file: SourceFile, keywordRes: KeywordRegExp[]): number {
  ...
  for (let i = 0; i < keywordRes.length; i++) {
    ...
    // count content occurrences (capped so one huge file can't dominate) using pre-compiled regex
    re.lastIndex = 0;
    let hits = 0;
    while (re.test(content) && hits < 20) {
      hits++;
    }
    score += hits;
  }

  for (let i = 0; i < SECURITY_HINTS_RES.length; i++) {
    const hintRe = SECURITY_HINTS_RES[i];
    if (hintRe.test(path)) score += 4;
    if (hintRe.test(content)) score += 1;
  }
...
export function packContext(bundle: SourceBundle, opts: PackOptions): PackedContext {
  ...
  // ⚡ BOLT OPTIMIZATION: Pre-compile keywords into case-insensitive, global RegExp objects
  // for fast, string-copy-free case-insensitive matching.
